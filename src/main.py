...
def run_analysis(video_path, output_video_path):
    ...
    #output_video_path = os.path.join(output_video_path, f"{video_path_no_ext}_coached.mp4")
    ...
    smooth_angle_buffer = deque(maxlen=5) # Buffer for smoothing the guidance angle
    ...
    while cap.isOpened():
        ...
        if np.all(np.sum(normalized_kps, axis=1) > 0):
            ...
            if len(keypoints_buffer) == 3:
                ...
                features = calculate_all_features(p_curr, p_prev, p_prev_prev, angles_buffer)
        ...
        if full_confidences[config.NOSE_IDX] > 0.2:
             draw_gait_prediction(frame, last_prediction, full_keypoints_xy)
...
if __name__ == '__main__':
    # Expects: python main.py <input_path> <output_path>
    if len(sys.argv) != 3:
    ...
    # You can set DEBUG mode to True for testing from the command line if you want
    config.DEBUG = True 
